...
調査が始まると専用ブラウザウィンドウが前面に表示されます。オーバーレイに表示された URL をブラウザのアドレスバーに入力するか、クリックしてコピーしてページを開いてください。ページを自由に操作して、ログインや CAPTCHA 解除を済ませてからボタンを押せます。TUI（ターミナル）への入力は不要です。

- **✅ キャプチャして続行**: そのページを取得して次へ進みます。
- **⏭ スキップ**: その URL を取得せずに次へ進みます。

リンクをクリックして別ページに移動した場合も、オーバーレイは自動的に再表示されます。
