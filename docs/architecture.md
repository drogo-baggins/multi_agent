...
│              ┌──────────┼──────────┐                │
│              │                     │                │
│        route_to_worker       route_to_manager       │
│              │                     │                │
│        ┌─────▼─────┐        ┌─────▼──────┐         │
│        │  Worker    │        │  Manager   │         │
│        │  Agent     │        │  Agent     │         │
...
### Decision 2: LLMベースのルーティング

Proxy Agentのカスタムツール（`route_to_worker` / `route_to_manager`）でルーティング。LLMがリクエストの性質を判断して適切なツールを呼び出す。

**理由**: PI toolkitのtool executionパターンに自然に乗り、自然言語の多様性に対応できる。ルールベースの分類より柔軟。
...
2. Proxy LLM → route_to_worker ツール呼び出しを判断
3. route_to_worker.execute():
   a. AgentRegistry.get("worker") → Worker Agent取得（遅延初期化）
   b. Worker Agent.prompt(message) → LLM呼び出し + ツール実行
   c. Worker Agent.waitForIdle() → 完了待ち
   d. 最終AssistantMessageからテキスト抽出 → ToolResult返却
...
2. Proxy LLM → route_to_manager ツール呼び出しを判断
3. route_to_manager.execute():
   a. AgentRegistry.get("manager") → Manager Agent取得
   b. Manager Agent.prompt(message) → LLM呼び出し
   c. Manager LLM → read_worker_config, read_work_product で現状分析
   d. Manager LLM → update_worker_config で設定変更
