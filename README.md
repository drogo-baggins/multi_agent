...
Chrome が自動起動する（CDP ポート 9222）。ページごとに確認ボックスが表示され、ページを確認したら **Enter キーを押す**だけで内容が取得される。

**ウィンドウ位置・サイズの設定（オプション）:**

```bash
CHROME_WINDOW_POSITION=1920,0   # ウィンドウ左上座標 x,y（例: 第2モニター左上）
CHROME_WINDOW_SIZE=1280,900     # ウィンドウサイズ width,height
```
